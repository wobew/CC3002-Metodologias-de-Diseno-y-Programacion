...
class DropPanelTest extends munit.FunSuite  {
  ...
  */
  ...
  override def beforeEach(context: BeforeEach): Unit = {
    ...
    //character.stars = randomNumberGenerator.nextInt(200)
  }
  ...
  test("A DropPanel should be able to add a character and remove him some stars") {
    panel.addCharacter(character)
    assert(panel.characters.contains(character))
  }

  test("A DropPanel should be able to remove a character") {
    panel.addCharacter(character)
    panel.removeCharacter(character)
    assert(!panel.characters.contains(character))
  }

  test("A DropPanel should drop some character stars correctly when its activated") {
    for (_ <- 1 to 20) {
      panel.addCharacter(character)
      character.addStars(4)
      panel.apply(character)
      assert(character.stars >= 0 && character.stars <= 4)
      var minus = character.stars
      character.removeStars(minus)
      panel.removeCharacter(character)
  ...
  test("If a player is not in the panel Apply does not affect them") {
    character.addStars(100)
    panel.apply(character)
    assertEquals(character.stars, 100)
  }
  ...
  test("A panel returns correctly it's name") {
    assertEquals(panel.name, "Drop Panel")
  }
}